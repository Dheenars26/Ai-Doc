...
import { generateGroundedAnswerStream } from '../services/aiService';
import { documentStore } from '../utils/documentStore';

/**
 * Handles real-time question answering via Server-Sent Events (SSE).
 */
export const askQuestionStream = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const stream = generateGroundedAnswerStream(docText, question, fileBuffer, mimeType, history);
    ...
    if (!res.headersSent) {
      res.status(500).json({
        success: false,
        message: (error as Error).message || 'Failed to initialize answer stream.',
      });
    } else {
      res.write(`data: ${JSON.stringify({ error: (error as Error).message || 'Stream interrupted.' })}\n\n`);
      res.end();
    }
  }
};

export default askQuestionStream;
